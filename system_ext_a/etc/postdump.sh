...
postdump_test_file=${sdcard}/.postdump.test.file
...
move_tlcore_system_to_crashdata()
{
    ...
    for s_file in $(ls $@) ; do
        ...
        install -p -o system -g vendor_crashdump -m $(stat -c %a $@/$s_file) $@/$s_file $partial_tlcore_dir/tlcore_system_0$idx
...
move_to_crashdump()
{
    ...
    if [ $? -eq 0 ] ; then
        ...
        log -p e -t postdump "failed to copy $src_dir/$@ $target_dir/$dir"
    fi
    if [ $is_exists = 1 ]; then
...
if [ "$variant" = "userdebug" ] ; then
    ...
    for i in $(seq 120); do
        ...
        elif ! echo "t" > ${postdump_test_file}; then
            # /sdcard/ can be readable but not writable during
            # 'checking' state
            log -p i -t postdump "waiting for creating ${postdump_test_file}, $i"
        else
            break
        fi
    done
    rm -f ${postdump_test_file}
